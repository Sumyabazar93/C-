...
//     point_update(ss, mid, i, inc, index * 2);
//     point_update(mid + 1, se, i, inc, index * 2 + 1);
...
vector<ll> blockers[1000006];
vector<ll> friends(1000006);
vector<ll> link(1000006);
vector<ll> sizel(1000006);
...
void solve()
{
    ll n, m, j, i, s, h, k, p, q, u, d;
    
    cin >> n >> m >> k;

    for (i = 0; i < n; i ++) link[i] = i;
    for (i = 0; i < n; i ++) sizel[i] = 1;
    ...
    for(i = 0; i < n; i ++) {
        ll ans = sizel[find(i)] - friends[i];
        for(j = 0; j < blockers[i].size(); j ++) {
            if(link[find(blockers[i][j])] == link[find(i)]) {
                ans --;
            }
        }
        cout << ans - 1 << " ";
